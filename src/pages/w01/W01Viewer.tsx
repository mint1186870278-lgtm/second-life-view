...
function AnnotationLegend({ groups }: { groups: readonly W01AnnotationGroup[] }) {
  const visibleGroups = groups.slice(0, 8)
  const hiddenCount = Math.max(0, groups.length - visibleGroups.length)
  return (
    <section className="w01-annotation-legend" aria-label="当前场景构件分组">
      <div className="w01-annotation-legend__heading"><strong>构件分组</strong><span>{groups.length} 组</span></div>
      <div className="w01-annotation-legend__rows">
        {visibleGroups.map((group) => (
          <span className={`w01-annotation-chip is-${group.category}`} key={group.id}>{group.name}<small>× {group.quantity}</small></span>
        ))}
        {hiddenCount > 0 && <span className="w01-annotation-chip is-more">另有 {hiddenCount} 组</span>}
      </div>
    </section>
  )
}
...
export function SceneViewer({
  ...
  const usesAnalysisAnnotations = scene.annotation_groups !== undefined
  const imageUrl = regeneration ? scene.annotated_image_url ?? scene.image_url : scene.image_url
  const imageAlt = regeneration && scene.annotated_image_url
    ? `${scene.name} YOLO 标注图`
    : `${scene.name} 360° 现场原图`
  return (
    <section className={`w01-viewer is-tone-${scene.visual_tone}`} aria-labelledby="w01-page-title" data-scene-id={scene.scene_id}>
      <div className="w01-scene-layer" style={{ transform: `scale(${zoom})` }}>
        <div
          className={`w01-panorama${imageUrl ? ' has-image' : ''}`}
          {...(!imageUrl ? { role: 'img', 'aria-label': `${scene.name} 360° 场景视觉占位` } : {})}
        >
          {imageUrl && <img className="w01-panorama-image" src={imageUrl} alt={imageAlt} />}
        </div>
        {regeneration && !usesAnalysisAnnotations && <HotspotLayer sceneId={scene.scene_id} hotspots={hotspots} projection={demoProjection} onOpenBatch={onOpenBatch} />}
      </div>
      <h1 className="sr-only" id="w01-page-title">再生视图</h1>
      <div className="w01-overlay-rail">
        <div className="w01-viewer-top-actions">
          <button type="button" className="w01-3d-entry" onClick={onOpenThreeD}>3d再生</button>
          <ViewerControls zoom={zoom} minZoom={minZoom} maxZoom={maxZoom} onZoomIn={onZoomIn} onZoomOut={onZoomOut} />
        </div>
        {regeneration && !usesAnalysisAnnotations && <div className="w01-overlay-stack">
          <ContextTaskCard tasks={tasks} projection={demoProjection} onOpenTask={onOpenTask} />
          <AssessmentInfoCard />
        </div>}
      </div>
      {regeneration && (usesAnalysisAnnotations
        ? <AnnotationLegend groups={scene.annotation_groups ?? []} />
        : <RegenerationPotentialCard projection={demoProjection.potential} />)}
    </section>
  )
}
      ...
      {scenes.map((scene, index) => (
        ...
        >
          <span className="w01-filmstrip-media" aria-hidden="true">
            {scene.image_url && <img src={scene.image_url} alt="" loading="lazy" />}
          </span>
