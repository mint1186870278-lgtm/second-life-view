...
function C03AnalysisContent() {
  ...
  const fallback = buildCreationAnalysisSummary(project.project_id, scenes, componentInstances, assessmentBatches, verificationItems)
  const sceneCount = analysisResult?.scene_count ?? fallback.sceneCount
  const componentCount = analysisResult?.component_count ?? fallback.componentCount
  const groupCount = analysisResult?.group_count ?? fallback.assessmentBatchCount
  const pendingEvidence = analysisResult?.pending_evidence_count ?? fallback.activeVerificationItemCount
  const processedScenes = analysisResult?.scenes ?? fallback.processedScenes.map((scene) => ({
    id: scene.scene_id,
    name: scene.name,
    ...
    <div className="creation-layout">
      <section className="panel creation-main-panel analysis-main">
        ...
        <section className="analysis-hero">
          ...
          <div>
            <h2>分析处理已完成</h2>
            <p>{sceneCount} 个场景 · {componentCount} 个构件 · {groupCount} 个分组</p>
            <span>所有任务已成功完成，可进入再生视图。</span>
          </div>
        </section>
        <div className="analysis-stages">
          <AnalysisStage icon={Image} title="素材预处理">{sceneCount} 个已接入场景已完成预处理，包括图像增强、格式标准化与空间信息提取。</AnalysisStage>
          <AnalysisStage icon={Box} title="构件识别与分组">识别 {componentCount} 个构件，形成 {groupCount} 组，已按构件类型进行初步分组。</AnalysisStage>
          <AnalysisStage icon={Crosshair} title="初步评估草案">生成路径建议与待核实事项，为后续评估提供依据。</AnalysisStage>
        </div>
        <div className="metrics-grid">
          <MetricCard icon={FileText} label="已识别构件" value={componentCount} unit="个" />
          <MetricCard icon={Crosshair} label="待核实事项" value={pendingEvidence} unit="项" accent />
          <MetricCard icon={FileText} label="评估草案" value={1} unit="份" />
        </div>
        <div className="section-heading section-heading--compact">
          <h2>已处理场景</h2><span>{processedScenes.length} / {sceneCount}</span>
        </div>
        <div className="processed-scenes">
          {processedScenes.map((scene, index) => (
            <article className="processed-scene" key={scene.id}>
              {scene.thumbnail_url ? <img src={scene.thumbnail_url} alt={`${scene.name} 处理完成预览`} /> : <MediaFallback label={`${scene.name} 媒体占位`} />}
